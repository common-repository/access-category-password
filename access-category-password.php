...
if( !function_exists('acpwd_options_settings'))  {
	function acpwd_options_settings(){
		...
		add_settings_field(
			'protectway',  //$id a unique id for the field
			__( 'The protection type', 'access-category-password' ), // the title for the field
			'acpwd_func_protectway',  // the function callback, to display the input box
		...
		add_settings_field(
			'cookieexp',
			__( 'Cookie expiration', 'access-category-password' ),
			'acpwd_func_cookieexp',
		...
		add_settings_field(
			'password',
			__( 'The password', 'access-category-password' ),
			'acpwd_func_password',
			'acpwd_setting_section',
			'acpwd_option_main'
        );
        ...
        add_settings_field(
			'excerpt_before_pass',
			__( 'Display excerpt before password form', 'access-category-password' ),
			'acpwd_func_excerpt_before_pass',
			'acpwd_setting_section',
			'acpwd_option_main'
        );
...
/** The way used to prtotect the categories **/
if( !function_exists('acpwd_func_protectway'))  {
	function acpwd_func_protectway() {
		 /* Get the option value from the database. */
		$options = get_option( 'acpwd_settings_options' );
		$protectway = (isset($options['protectway'])) ? $options['protectway'] : 'session' ;
		/* Echo the field. */ ?>
		<label for="protectway" ><?php _e( 'Way used to protect', 'access-category-password' ); ?></label>
		<select name="acpwd_settings_options[protectway]" id="protectway">
				<option value="session"<?php if ($protectway == 'session') echo ' selected="selected"' ?>><?php _e( 'Sesssion', 'access-category-password' ); ?></option>
				<option value="cookie"<?php if ($protectway == 'cookie') echo ' selected="selected"' ?>><?php _e( 'Cookie', 'access-category-password' ); ?></option>
		</select>
		<p class="description">
		    <?php _e( 'You can choose wich way you want to use to protect the categories. <code>Session</code> is recommended but doesn\'t work when you\'re caching the content of your blog (using for example <a target="_blank" href="https://fr.wordpress.org/plugins/search/cache/">these caching plugins</a>), you need to reenter the password each time you launch your browser, this is the <strong>safer</strong> process. <code>Cookie</code> will always work, you don\'t need to type the password each time you want to access to the resource, but this is the <strong>less secure</strong> way.', 'access-category-password' ); ?>
		</p>
		<!-- javaScript behaviour when switching -->
		<script>
		jQuery(document).ready(function() {
			if (jQuery("#protectway").val() == 'session') {
				jQuery("#cookieset").parent("td").parent("tr").fadeOut(400);
			} else {
				jQuery("#cookieset").parent("td").parent("tr").fadeIn(400);
			};
			jQuery("#protectway").on("change", function() {
				if (jQuery(this).val() == 'session') {
					jQuery("#cookieset").parent("td").parent("tr").fadeOut(400);
				} else {
					jQuery("#cookieset").parent("td").parent("tr").fadeIn(400);
				}
...
/** The way used to prtotect the categories **/
if( !function_exists('acpwd_func_cookieexp'))  {
	function acpwd_func_cookieexp() {
		 /* Get the option value from the database. */
		$options = get_option( 'acpwd_settings_options' );
		$cookieexp = (isset($options['cookieexp'])) ? $options['cookieexp'] : 311040000 ;
		/* Echo the field. */
		?>
		<div id="cookieset">
		<label for="cookieexp" ><?php _e( 'Expiration time of the cookie', 'access-category-password' ); ?></label>
		<select name="acpwd_settings_options[cookieexp]" id="cookieexp">
				<option value="2592000"<?php if ($cookieexp == 2592000) echo ' selected="selected"' ?>><?php _e( '1 month', 'access-category-password' ); ?></option>
				<option value="7776000"<?php if ($cookieexp == 7776000) echo ' selected="selected"' ?>><?php _e( '3 months', 'access-category-password' ); ?></option>
				<option value="15552000"<?php if ($cookieexp == 15552000) echo ' selected="selected"' ?>><?php _e( '6 months', 'access-category-password' ); ?></option>
				<option value="31104000"<?php if ($cookieexp == 31104000) echo ' selected="selected"' ?>><?php _e( '1 year', 'access-category-password' ); ?></option>
				<option value="62208000"<?php if ($cookieexp == 62208000) echo ' selected="selected"' ?>><?php _e( '2 years', 'access-category-password' ); ?></option>
				<option value="124416000"<?php if ($cookieexp == 124416000) echo ' selected="selected"' ?>><?php _e( '4 years', 'access-category-password' ); ?></option>
				<option value="311040000"<?php if ($cookieexp == 311040000) echo ' selected="selected"' ?>><?php _e( '10 years', 'access-category-password' ); ?></option>
		</select>
		<p class="description">
		    <?php _e( 'Time after first set the Cookie will expire.', 'access-category-password' ); ?>
        </p>
		</div>
    <?php }
}
...
if( !function_exists('acpwd_func_password'))  {
	function acpwd_func_password() {
		...
		<input type="password" id="password" name="acpwd_settings_options[password]" value="<?php echo $password ?>" />
		<p class="description">
...
/** Excerpt before password form **/
if( !function_exists('acpwd_func_excerpt_before_pass'))  {
	function acpwd_func_excerpt_before_pass(){
	/* Get the option value from the database. */
		$options = get_option( 'acpwd_settings_options' );
		$excerpt_before_pass = (isset($options['excerpt_before_pass'])) ? $options['excerpt_before_pass'] : 0;
		$excerpt_before_pass_length = (isset($options['excerpt_before_pass_length'])) ? $options['excerpt_before_pass_length'] : 10;
		$excerpt_before_pass_ending_string = (isset($options['excerpt_before_pass_ending_string'])) ? $options['excerpt_before_pass_ending_string'] : ' [&hellip;]';
		/* Echo the field. */ ?>
		<div>
            <p><input id="excerpt_before_pass" type="checkbox" name="acpwd_settings_options[excerpt_before_pass]" value="1"<?php if (isset($excerpt_before_pass) && $excerpt_before_pass == 1) echo ' checked'; ?> /> <?php _e( 'Add a customized excerpt before password form', 'access-category-password' ); ?> <span id="excerpt_before_pass_set"><?php _e( 'containing the', 'access-category-password' ); ?> <input type="number" name="acpwd_settings_options[excerpt_before_pass_length]" style="width: 70px;" value="<?php echo $excerpt_before_pass_length ?>"> <?php _e( 'first words and ending with ', 'access-category-password' ); ?><input type="text" name="acpwd_settings_options[excerpt_before_pass_ending_string]" value="<?php echo $excerpt_before_pass_ending_string ?>"></span></p>
		<p class="description">
		    <?php _e( 'Check this if you want to show a costumized post excerpt before the password form. You can set the length (in words) of this excerpt and the string that should display at the end.', 'access-category-password' ); ?>
        </p>
        </div>
		<!-- javaScript behaviour when switching -->
		<script>
		jQuery(document).ready(function() {
			if (jQuery("#excerpt_before_pass").prop("checked")) {
				jQuery("#excerpt_before_pass_set").fadeIn(400);
			} else {
				jQuery("#excerpt_before_pass_set").fadeOut(400);
			};
			jQuery("#excerpt_before_pass").on("change", function() {
				if (jQuery("#excerpt_before_pass").prop("checked")) {
					jQuery("#excerpt_before_pass_set").fadeIn(400);
				} else {
					jQuery("#excerpt_before_pass_set").fadeOut(400);
...
if( !function_exists('acpwd_func_output_styling'))  {
	function acpwd_func_output_styling(){
		$options = get_option( 'acpwd_settings_options' );
		$acpwd_default_css = '* Custom css for Access Category Password form */
		.acpwd-container {
		...
		$output_styling = (isset($options['output_styling']) && $options['output_styling'] != '') ? $options['output_styling'] : $acpwd_default_css;
?>
<p><?php _e( 'You can style the form that replace the content of a protected article by using the <strong style="color: #0073AA">classes</strong> you can see in the code below:', 'access-category-password' ) ?><br>.</p>
<pre style="width: 90%; overflow: auto; padding: 1% 2%; margin: 1% 0; background: #FFF; border: #ddd 1px solid;">&lt;div class&equals;&quot;<span style="color: #0073AA">acpwd-container</span>&quot;&gt;&NewLine;   &lt;p class&equals;&quot;<span style="color: #0073AA">acpwd-added-excerpt</span>&quot;&gt;Here comes the Excerpt if enabled&hellip;&lt;&sol;p&gt;&NewLine;   &lt;p class&equals;&quot;<span style="color: #0073AA">acpwd-info-message</span>&quot;&gt;This content has restricted access&comma; please type the password below and get access&period;&lt;&sol;p&gt;&NewLine;   &lt;form class&equals;&quot;<span style="color: #0073AA">acpwd-form</span>&quot; action&equals;&quot;&quot; method&equals;&quot;post&quot;&gt;&NewLine;      &lt;input class&equals;&quot;<span style="color: #0073AA">acpwd-pass</span>&quot; type&equals;&quot;password&quot; name&equals;&quot;acpwd-pass&quot;&gt;&NewLine;      &lt;input class&equals;&quot;<span style="color: #0073AA">acpwd-submit</span>&quot; type&equals;&quot;submit&quot; value&equals;&quot;Get access&quot;&gt;&NewLine;   &lt;&sol;form&gt;&NewLine;   &lt;p class&equals;&quot;<span style="color: #0073AA">acpwd-error-message</span>&quot;&gt;Sorry&comma; but this is the wrong password&period;&lt;&sol;p&gt;&NewLine;&lt;&sol;div&gt;</pre>
<p class="description">
	<?php _e( 'Below the css you can change to fit to your blog styling.', 'access-category-password' ) ?>
</p>
<textarea name="acpwd_settings_options[output_styling]" style="width: 95%; height: 300px;"><?php echo $output_styling ?></textarea>
...
if( !function_exists('acpwd_options_validate'))  {
	...
	/** Protection type */
	if($input['protectway'] == 'session') {
		// Remove the cookie if existing
		setcookie('acpwd_cookie', '', time() - 3600, '/');
		if (isset($_COOKIE['acpwd_cookie'])) unset($_COOKIE['acpwd_cookie']);
	} else {
		// unset the session if existing
		if (isset($_SESSION['acpwd_session'])) {
			unset($_SESSION['acpwd_session']);
			session_destroy ();
		}
	}
	$options['protectway'] = $input['protectway'];

	/** Expiration time for cookie */
	$options['cookieexp'] = $input['cookieexp'];
  ...
  /** Only single validation **/
  if ( ! isset( $input['only_single'] ) )
      $input['only_single'] = 0;
  $options['only_single'] = $input['only_single'];

  /** Show the excerpt validation **/
  if ( ! isset( $input['excerpt_before_pass'] ) )
      $input['excerpt_before_pass'] = 0;
  $options['excerpt_before_pass'] = $input['excerpt_before_pass'];
  $options['excerpt_before_pass_length'] = $input['excerpt_before_pass_length'];
  $options['excerpt_before_pass_ending_string'] = wp_filter_nohtml_kses( esc_attr($input['excerpt_before_pass_ending_string']));
  ...
  /** clean input placeholder text HTML not allowed */
	...
	/** css output */
	$options['output_styling'] = $input['output_styling'];
...
// Adding customized styling to <head>

function acpwd_head_styling() { 
	$options = get_option( 'acpwd_settings_options' );
	$acpwd_default_css = '* Custom css for Access Category Password form */
	.acpwd-container {
	...
	$output_styling = (isset($options['output_styling']) && $options['output_styling'] != '') ? '<style type="text/css">'.$options['output_styling'].'</style>' : '<style type="text/css">'.$acpwd_default_css.'</style>';
	echo $output_styling;
}
add_action( 'wp_head', 'acpwd_head_styling' );

/* Start and destroy sessions */
add_action('init', 'acpwdStartSession', 0);
add_action('wp_logout', 'acpwdEndSession');
add_action('wp_login', 'acpwdStartSession', 0);

function acpwdStartSession() {
	// Start session only if this is the way we want to authenticate...
	$acpwd_options = get_option('acpwd_settings_options');
	$protectway = (isset($acpwd_options['protectway'])) ? $acpwd_options['protectway'] : 'session';
	if (!session_id() && $protectway == 'session') {
		session_start();
	}
}
...
function acpwd_mdp_check() {
    ...
    if(isset($_POST['acpwd-pass'])) {
		// Checking password
		$acpwd_options = get_option('acpwd_settings_options');
		$cookieexp = (isset($acpwd_options['cookieexp'])) ? $acpwd_options['cookieexp'] : 311040000;
		$protectway = (isset($acpwd_options['protectway'])) ? $acpwd_options['protectway'] : 'session';
        if(crypt($_POST['acpwd-pass'], $_POST['acpwd-pass']) == $acpwd_options['password']) {
			if($protectway == 'session') {
				...
				if (!isset($_COOKIE['acpwd_cookie'])) setcookie('acpwd_cookie', 1, time() + $cookieexp, '/');
				$_POST['acpwd_cookie_ok'] = true;
			}
        }
        else {
			...
			if($protectway == 'session') {
				...
				setcookie('acpwd_cookie', 0, time() - 3600, '/');
				unset($_COOKIE['acpwd_cookie']);
				$_POST['acpwd_cookie_ok'] = false;
			}
        }
    }
}
add_action('init', 'acpwd_mdp_check', 1);

/* Displaying the password form or the feed replacement sentence for post or excerpt content */
function acpwd_frontend_changes($content) {
	...
	$excerpt_before_pass = (isset($acpwd_options['excerpt_before_pass'])) ? $acpwd_options['excerpt_before_pass'] : 0;
	$excerpt_before_pass_length = (isset($acpwd_options['excerpt_before_pass_length'])) ? $acpwd_options['excerpt_before_pass_length'] : 10;
	$excerpt_before_pass_ending_string = (isset($acpwd_options['excerpt_before_pass_ending_string'])) ? $acpwd_options['excerpt_before_pass_ending_string'] : ' [&hellip;]';
	$added_excerpt = ($excerpt_before_pass == 1) ? '<p class="acpwd-added-excerpt">'.wp_trim_words( $content, $excerpt_before_pass_length, $excerpt_before_pass_ending_string ).'</p>'."\n" : '';
	$info_message= (isset($acpwd_options['info_message']) && $acpwd_options['info_message'] != '') ? '<p class="acpwd-info-message">'.stripslashes($acpwd_options['info_message']).'</p>' : '<p class="acpwd-info-message">'.__('This content has restricted access, please type the password below and get access.', 'access-category-password').'</p>';
	$validation_button = (isset($acpwd_options['valid_button_text']) && $acpwd_options['valid_button_text'] != '') ? '<input class="acpwd-submit" type="submit" value="'.$acpwd_options['valid_button_text'].'">' : '<input class="acpwd-submit" type="submit" value="'.__('Get access', 'access-category-password').'">';
	$input_placeholder = (isset($acpwd_options['input_placeholder'])) ? stripslashes($acpwd_options['input_placeholder']) : '';
	if ( in_category($impacted_categories) ) {
		// For the following test : Session case || Cookie case || Single post case || Allowed users case
		if (($protectway == 'session' && isset($_SESSION['acpwd_session']) && $_SESSION['acpwd_session'] == 1) || (($protectway == 'cookie' && isset($_POST['acpwd_cookie_ok']) && $_POST['acpwd_cookie_ok'])) || ($protectway == 'cookie' && isset($_COOKIE['acpwd_cookie']) && $_COOKIE['acpwd_cookie'] == 1) || (isset($acpwd_options['only_single']) && $acpwd_options['only_single'] == 1 && !is_single()) || in_array($u_role, $allowed_users)) {
			$new_content = $content;
		} else {
			if (is_feed()) {
				// Feed content replacement
				$new_content = stripslashes($acpwd_options['feed_desc_text']) ;
			} elseif (is_main_query()) {
				// Post or excerpt content replacement
				$new_content = '<div class="acpwd-container" id="acpwd-'.get_the_ID().'">'."\n";
				$new_content .= $added_excerpt;
				//$new_content .= '<p style="color: red">content###'.$content.'####</p>';
				$new_content .= $info_message."\n";
				$new_content .= '<form class="acpwd-form" action="'.$_SERVER['REQUEST_URI'].'#acpwd-'.get_the_ID().'" method="post">'."\n";
				$new_content .= "\t".'<input class="acpwd-pass" type="password" name="acpwd-pass" placeholder="'.$input_placeholder.'">';
				$new_content .= $validation_button."\n";
				$new_content .= '</form>'."\n";
				if (isset ($_POST['acpwd-msg']))
					$new_content .= $_POST['acpwd-msg'];
				$new_content .= '</div>';
			}
		}
	} else {
